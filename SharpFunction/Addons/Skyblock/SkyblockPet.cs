...
namespace SharpFunction.Addons.Skyblock
{
    ...
    public sealed class SkyblockPet
    {
        ...
        private RawText fullDesc;
        private RawText name;

        private Dictionary<ItemRarity, int> rar = new()
        {
        ...
        public SkyblockPet(string name, string skullOwner,  ItemRarity rarity, PetType type)
        {
            ...
            pn.Append($"{name}", SkyblockEnumHelper.GetRarityColor(rarity));
            this.name.AddField(pn);
            fullDesc.AddField($"{EnumHelper.GetStringValue(type)}", Color.DarkGray, RawTextFormatting.Straight);
        }
        /// <summary>
        ...
        public Give Generate()
        {
            ...
            if (stats.Any(predicate))
            {
                ...
                IEnumerable<KeyValuePair<string, int?>> parsed = stats.Where(predicate);
                foreach(KeyValuePair<string, int?> pair in parsed)
                {
                    SuperRawText p = new();
                    p.Append(pair.Key+':', Color.Gray);
                    string stat = percented.Contains(pair.Key) ? $" +{pair.Value}%" : $" +{pair.Value}";
            ...
            {
                foreach(PetAbility abil in Abilities)
                {
                    var d = abil.GetAbility();
                    foreach(SuperRawText line in d.Lines)
                    {
                        fullDesc.AddField(line);
                    }
                }
            }
            fullDesc.AddField("");
            ...
            fullDesc.AddField("");
            fullDesc.AddField($"{EnumHelper.GetStringValue(Rarity)}", SkyblockEnumHelper.GetRarityColor(Rarity), RawTextFormatting.Straight, RawTextFormatting.Bold);
    ...
    public sealed class PetAbility
    {
        ...
        public AdvancedDescription GetAbility()
        {
            ...
            {
                foreach(SuperRawText line in AbilityDescription.Lines)
                {
                    d.Append(line);
                }
            }
