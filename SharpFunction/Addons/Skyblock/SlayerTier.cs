...
namespace SharpFunction.Addons.Skyblock
{
    ...
    {
        private readonly dynamic col = new Dictionary<int, Color>
        {
            {1, Color.Green},
            {2, Color.Yellow},
            {3, Color.Red},
        ...
        private readonly dynamic num = new Dictionary<int, string>
        {
            {1, "I"},
            {2, "II"},
            {3, "III"},
        ...
        /// <summary>
        ///     Represents item to give with command. E.G. Rotten flesh <br />for revenant horror and ender pearl for voidgloom
        ///     seraph
        /// </summary>
        ...
        public void Compile()
        {
            ...
            upper.AddField(" ");
            hp.Append("Health: ", Color.Gray);
            hp.Append($"{HP.ToString("N0", new CultureInfo("en-US", false).NumberFormat)}{SkyblockHelper.HEALTH}",
                Color.Red);
            dps.Append("Damage: ", Color.Gray);
            dps.Append($"{DPS.ToString("N0", new CultureInfo("en-US", false).NumberFormat)}", Color.Red);
            dps.Append(" per second", Color.Gray);
            upper.AddField(hp);
            upper.AddField(dps);
            upper.AddField(" ");
            ...
            reward.Append("Reward: ", Color.Gray);
            reward.Append(
                $" {XP} {RawMobName.First().ToString().ToUpper() + string.Concat(RawMobName.Skip(1))} Slayer XP",
                Color.LightPurple);
            upper.AddField(reward);
            upper.AddField(" + Boss drops", Color.DarkGray, RawTextFormatting.Straight);
            upper.AddField(" ");
            SuperRawText cost = new();
            cost.Append("Cost to start: ", Color.Gray);
