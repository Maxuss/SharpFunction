...
namespace SharpFunction.Addons.Skyblock
{
    ...
    public sealed class SkyblockLocation
    {
        ...
        {
            string fb = "--------------------------------------------\\n";
            SuperRawText filler = new SuperRawText().Append(fb, Color.Green, RawTextFormatting.Bold);
            SuperRawText nz = new();
            ...
            string f = full.RawJSON.Replace("'", "").Replace("[", "").Replace("]", "");
            string cmd = $"/tellraw @p [\"\",{f}]";
            return cmd;
        }
    }
}
