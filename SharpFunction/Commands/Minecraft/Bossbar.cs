...
namespace SharpFunction.Commands.Minecraft
{
    ...
    public sealed class Bossbar : ICommand
    {
        ...
        ///     5. <see cref="BossbarSet.Style" /> > style string. Allowed values:
        ...
        public void Compile(string id, BossbarSet set, object extra)
        {
            try
            {
                switch (set)
                {
                    ...
                    case BossbarSet.Name:
                        var name = (string) extra;
                        Compiled = $"/bossbar set {id} name {name}";
                        break;
                    case BossbarSet.Players:
                        var targets = (EntitySelector) extra;
                        Compiled = $"/bossbar set {id} players {targets.String()}";
                        break;
                    case BossbarSet.Style:
                        string[] possible = {"notched_6", "notched_10", "notched_12", "notched_20", "progress"};
                        var style = (string) extra;
                        if (!possible.Contains(style))
                            throw new ArgumentException(
                                $"{nameof(extra)} is not allowed style. Allowed styles: (notched_6|notched_10|notched_12|notched_20|progress)");
                        Compiled = $"/bossbar set {id} style {style}";
                        break;
                    case BossbarSet.Value:
                        var val = (short) extra;
                        if (val > 100 || val < 0)
                            throw new ArgumentException($"{nameof(extra)} is more than 100 or less than 0.");
                        Compiled = $"/bossbar set {id} value {val}";
                        break;
                    case BossbarSet.Visible:
                        var visible = (bool) extra;
            ...
            {
                throw new ArgumentException($"{nameof(extra)} is wrong type. More info: {e.Message}");
            }
        }
    }
    ...
    public readonly struct BossbarColor
    {
        ...
        public readonly string Pink => "pink";
        public readonly string Purple => "purple";
        public readonly string Red => "red";
        public readonly string White => "white";
        public readonly string Yellow => "yellow";
    }
}